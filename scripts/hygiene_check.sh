...
for f in "${FILES[@]}"; do
  [[ -f "$f" ]] || continue
  if hits="$(grep -nE '(/Users/[A-Za-z0-9._-]+|/home/[A-Za-z0-9._-]+)' "$f" 2>/dev/null)"; then
...
if [[ "$MODE" != "staged" ]]; then
  scan_paths=("$ROOT")
  if [[ -n "${HYGIENE_SIBLING_PROJECTS:-}" ]]; then
    for sp in ${HYGIENE_SIBLING_PROJECTS}; do
      [[ -d "$sp" ]] && scan_paths+=("$sp")
    done
  fi
